...
import { NormalInput, Select, TextArea } from "components/Input"
import {NormalSelect} from "components/Input/Select"
...
const SendMessage: React.FC<IProps> = ({ }) => {
    ...
    const [value,setValue] = React.useState<IRates>()
            ...
            new Promise((resolve, reject) => {
                resolve(dispatch(addTransaction({
                    ...
                    ...(selectedTransfer === "gift" && {
                        ...
                        subCategory:values.subCardCategory
                    })
                }, dialog)))
            }).then(() => {
    ...
    const handleChatSubmit = async (values: messageForm, { ...action }: any) => {
        const chatMessageRef = firestore.collection("users").doc(currentChatProfile.id)
    ...
    const getCardSelected = (arg:any) => {
        return arg.cardCategory || ""
    }
    const getSubCardSelected = (arg:any) => {
        return arg.name || ""
    }
    ...
    return (
        <>
            <Dialog open={open} handleToggle={handleToggle} title="Create a New Transaction"
                dialogAction={
                    <>
                            ...
                            {(formikProps: FormikProps<transactionMessageForm>) => {
                                ...
                                return (
                                    <div className="flex flex-col justify-center">
                                        ...
                                        {
                                            selectedTransfer === "gift" &&
                                            <Box className="mx-auto w-3/4 space-y-3">
                                                <NormalSelect className="w-full" options={rates} name="cardCategory"
                                                    label="Select category" value={value} setValue={setValue}
                                                    getLabel={getCardSelected} />
                                                <Select className="w-full" options={subCard} label="Select category"
                                                    disabled={!subCard.length}
                                                    name="subCardCategory" getLabel={getSubCardSelected} />
                                            </Box>
                                        }
